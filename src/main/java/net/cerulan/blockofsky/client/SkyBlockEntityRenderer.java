...
public class SkyBlockEntityRenderer implements BlockEntityRenderer<SkyBlockEntity> {
    ...
    @Override
    public void render(SkyBlockEntity blockEntity, float f, PoseStack poseStack,
                       MultiBufferSource multiBufferSource, int i, int j) {
        if (!blockEntity.getBlockState().getValue(SkyBlock.ACTIVE)) return;

        ResourceLocation dimId = blockEntity.getDimensionId();
        RenderType renderType = SkyRenderManager.getRenderType(dimId);

        Matrix4f m4f = poseStack.last().pose();
        renderCube(blockEntity, m4f, multiBufferSource.getBuffer(renderType));

        SkyRenderManager.requestDimensionRender(dimId);
    }
    ...
    private void renderFace(SkyBlockEntity entity, Matrix4f matrix, VertexConsumer buffer,
                             float f, float g, float h, float i, float j, float k, float l,
                             float m, Direction direction) {
